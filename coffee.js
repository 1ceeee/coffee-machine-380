'use strict';
let state = "waiting"; //3 стадии: waiting  cooking   ready

let cupImg = document.querySelector(".coffee-cup img");
let progressBar = document.querySelector(".progress-bar");

cupImg.onclick = takeCoffee;

function takeCoffee(){
  if(state != "ready"){ return; }
  state = "waiting";
  cupImg.style.opacity = 0;
  //cupImg.style.cursor = "";
  cupImg.classList.add("d-none");
  changeDisplayText("Выберите кофе");
  progressBar.style.width = 0;
  
}


function buyCoffee(name, price, element){
  if(state != "waiting"){ return; }
  
      //console.log([name,price,element]);
  let balanceInput = document.querySelector("input[placeholder='Баланс']"); //поиск с помощью селектора атрибутов
      //let balane = balanceInput.value; - присваивать не будет, т.к. так не сможем менять значение на странице...
  if(+balanceInput.value < price){
    //alert("Недостаточно средств");
    changeDisplayText("Недостаточно средств");
    balanceInput.style.border = "2px solid red";
  }else{
    balanceInput.value -= price;
    balanceInput.style.border = ""; //если пустая строка - то стиль меняется на тот, что был ранее !
    state = "cooking";
    cookCoffee(name,element);
  }
  //console.log(balanceInput);
}

//console.log(progressBar);
//alert("Ваш " + name + " готовится");
  
function cookCoffee(name, buttonElement){
  changeDisplayText("Ваш " + name + " готовится");
  //let progressBar = document.querySelector(".progress-bar");
  let buttonImg = buttonElement.querySelector("img");
  //let cupSrc = buttonImg.src;                 // абсолютный путь - не будем тут использовать
  let cupSrc = buttonImg.getAttribute('src');   // относительный путь
  
  //let cupImg = document.querySelector(".coffee-cup img");
  cupImg.setAttribute('src', cupSrc);
  cupImg.classList.remove("d-none");
  
  //console.log(cupSrc);
  
  let i=0;
  let interval = setInterval(function(){
    i++;
    progressBar.style.width = i + "%";
    cupImg.style.opacity = i + "%";
    
    if (i==110){  //достаточно 100, просто чтобы текст менялся чуть позже...
      clearInterval(interval);
      changeDisplayText("Ваш " + name + " готов!");
      cupImg.style.cursor = "pointer";
      state = "ready";
    }
  }, 20);
  
}

function changeDisplayText(text){
  let displayText = document.querySelector(".display-text");
  displayText.innerHTML = text;
  
}

